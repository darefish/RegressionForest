...
//****************************************************************************************************
//FUNCTION:
void hiveRegressionForest::hiveOutputOOBAndLeafInfo(unsigned int vForestId)
{
	const CRegressionForest* pForest = CRegressionForestPool::getInstance()->fetchForest(vForestId);
	_ASSERTE(pForest);

	std::ofstream File("OOBInLeafNode.csv", std::ios::out);
	_ASSERTE(File.is_open());
		
	for (int i = 0; i < pForest->getNumOfTrees(); ++i)
	{
		std::vector<int> OOBInLeafNode;
		pForest->getTreeAt(i)->fetchOOBInLeafNode(OOBInLeafNode);
		
		std::sort(OOBInLeafNode.begin(), OOBInLeafNode.end());
		auto Itr = std::unique(OOBInLeafNode.begin(), OOBInLeafNode.end());
		
		File << OOBInLeafNode.size() << std::endl;
		for (int i = 0; i < OOBInLeafNode.size(); ++i)
		{
			File << OOBInLeafNode[i] << ",";
			if (i % 100 == 99 && i != 0) File << "\n";
		}
		File << std::endl;

		int Length = std::distance(OOBInLeafNode.begin(), Itr);
		OOBInLeafNode.resize(Length);
