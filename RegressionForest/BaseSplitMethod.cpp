...
//****************************************************************************************************
//FUNCTION:
bool INodeSpliter::splitNode(CNode* vCurrentNode, const std::pair<int, int>& vBootstrapRange, const std::vector<int>& vCurrentFeatureIndexSubSet, std::vector<int>& vioBootstrapIndex, int& voRangeSplitPos)
{
	// NOTES : 初始化为-1，如果执行子特征划分查找以后仍然为-1，则表示：
	//		   选择的子特征集合中各子特征所有的值均相等
	SSplitHyperplane AxisAlignedSplitHyperplane({-1, FLT_MAX});
	__findBestSplitHyperplane(vioBootstrapIndex, vBootstrapRange, vCurrentFeatureIndexSubSet, AxisAlignedSplitHyperplane);
	if (AxisAlignedSplitHyperplane.m_AxisAlignedSplitHyperplane.first == -1) return false;
	
	voRangeSplitPos = __processBootstrapRange(vBootstrapRange, AxisAlignedSplitHyperplane, vioBootstrapIndex);
...
//********************************************************************************************************
//FUNCTION:
void INodeSpliter::_generateSortedFeatureResponsePairSetV(const std::vector<int>& vBootstrapIndex, const std::pair<int, int>& vBootstrapRange, unsigned int vFeatureIndex, std::vector<std::pair<float, float>>& voSortedFeatureResponseSet)
{
...
//****************************************************************************************************
//FUNCTION:
void INodeSpliter::__findBestSplitHyperplane(const std::vector<int>& vBootstrapIndex, const std::pair<int, int>& vBootstrapRange, const std::vector<int>& vFeatureIndexSubset, SSplitHyperplane& voSplitHyperplane)
{
...
//****************************************************************************************************
//FUNCTION:
int INodeSpliter::__processBootstrapRange(const std::pair<int, int>& vBootstrapRange, SSplitHyperplane& vSplitHyperplane, std::vector<int>& vioBootstrapIndex)
{
	const CTrainingSet* pTrainingSet = CTrainingSet::getInstance();
	...
	while (L <= R)
	{		
		IsCurLInLeftSpace = vSplitHyperplane.IsInstanceInLeftSpace(CTrainingSet::getInstance()->getFeatureInstanceAt(vioBootstrapIndex[L]));
		IsCurRInLeftSpace = vSplitHyperplane.IsInstanceInLeftSpace(CTrainingSet::getInstance()->getFeatureInstanceAt(vioBootstrapIndex[R]));
		...
		else
		{
			...
			{
				std::swap(vioBootstrapIndex[L], vioBootstrapIndex[R]);
				L++;
			}
			R--;
		}
